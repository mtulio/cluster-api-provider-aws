...
func TestAWSMachineReconciler(t *testing.T) {
	...
	t.Run("Reconciling an AWSMachine", func(t *testing.T) {
		...
		t.Run("when BYO IP", func(t *testing.T) {
			var instance *infrav1.Instance
			secretPrefix := "test/secret"

			t.Run("with Public IPv4 Pool, should succeed", func(t *testing.T) {
				g := NewWithT(t)
				awsMachine := getAWSMachine()
				setup(t, g, awsMachine)
				defer teardown(t, g)
				...
				ec2Svc.EXPECT().ReconcileElasticIP(gomock.Any(), gomock.Any()).Return(false, nil).AnyTimes()
			...
			t.Run("with pre-allocated EIP, should succeed", func(t *testing.T) {
				g := NewWithT(t)
				awsMachine := getAWSMachine()
				setup(t, g, awsMachine)
				defer teardown(t, g)
				...
				ec2Svc.EXPECT().ReconcileElasticIP(gomock.Any(), gomock.Any()).Return(false, nil).AnyTimes()
				ms.AWSMachine.Spec.PublicIP = aws.Bool(true)
