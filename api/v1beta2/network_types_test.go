	...
	stubNetworkTypeSubnetsAvailabilityZone = []*SubnetSpec{
		{
			...
			IsPublic:         false,
			ZoneType:         ptr.To(ZoneType(ZoneTypeAvailabilityZone)),
		},
		{
	...
	stubNetworkTypeSubnetsLocalZone = []*SubnetSpec{
		{
			...
			IsPublic:         false,
			ZoneType:         ptr.To(ZoneType(ZoneTypeLocalZone)),
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := s.IsEdge(); got != tt.want {
				t.Errorf("SubnetSpec.IsEdge() = %v, want %v", got, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !cmp.Equal(s, tt.want) {
				t.Errorf("SubnetSpec.SetZoneInfo() \n diff: %#+v", cmp.Diff(s, tt.want))
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.subnets.IDsWithEdge(); !cmp.Equal(got, tt.want) {
				t.Errorf("Subnets.IDsWithEdge() Error diff:\n%v", cmp.Diff(got, tt.want))
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.subnets.FilterPrivate(); !cmp.Equal(got, tt.want) {
				t.Errorf("Subnets.FilterPrivate()\ngot: %#+v", cmp.Diff(got, tt.want))
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.subnets.GetUniqueZones(); !cmp.Equal(got, tt.want) {
				t.Errorf("Subnets.GetUniqueZones()got:\b %v", cmp.Diff(got, tt.want))
			}
		})
	}
}
