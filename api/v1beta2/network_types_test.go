...
func TestPublicIpv4PoolFallbackOrder_String(t *testing.T) {
	tests := []struct {
		name string
		r    PublicIpv4PoolFallbackOrder
		want string
	}{
		{
			name: "ensure amazon-pool",
			r:    PublicIpv4PoolFallbackOrder(PublicIpv4PoolFallbackOrderAmazonPool),
			want: "amazon-pool",
		},
		{
			name: "ensure none fallback",
			r:    PublicIpv4PoolFallbackOrder(PublicIpv4PoolFallbackOrderNone),
			want: "none",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.r.String(); got != tt.want {
				t.Errorf("PublicIpv4PoolFallbackOrder.String() = %v, want %v", got, tt.want)
			}
		})
	}
}
